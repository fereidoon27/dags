"""
Airflow HA Infrastructure Health Monitor - Version 200
Fixes:
- Eliminated duplicate log entries
...
- Shortened task names for better readability
"""
...
# Service name mappings for shorter task IDs
SERVICE_ABBREV = {
...
def fetch_journalctl_logs(ip: str, service: str, hostname: str, minutes_back: int = 2) -> str:
    ...
    try:
        if is_local_check:
            ...
            cmd = f"ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no rocky@{ip} \"sudo journalctl -u {service} --since '{minutes_back} minutes ago' --no-pager -n 150 -o short-precise\""
...
def fetch_system_resources(ip: str, hostname: str) -> Dict:
    ...
    try:
        # Properly escape the awk commands - use \\" for quotes inside the command
        cpu_cmd = "top -bn1 | grep 'Cpu(s)' | awk '{print $2}' | cut -d'%' -f1"
        mem_cmd = "free -m | awk 'NR==2{printf \\\"%s %s %.2f\\\", $3,$2,$3*100/$2 }'"
        disk_cmd = "df -h / | awk 'NR==2{printf \\\"%s %s %s\\\", $3,$2,$5}'"
        top_cpu_cmd = "ps aux --sort=-%cpu | head -6 | tail -5 | awk '{printf \\\"%s|%s|%s\\\\n\\\", $11, $3, $4}'"
        top_mem_cmd = "ps aux --sort=-%mem | head -6 | tail -5 | awk '{printf \\\"%s|%s|%s\\\\n\\\", $11, $3, $4}'"
        
        full_cmd = f"{cpu_cmd} && echo '---' && {mem_cmd} && echo '---' && {disk_cmd} && echo '---' && {top_cpu_cmd} && echo '---' && {top_mem_cmd}"
        
        if not is_local_check:
            # Need to escape quotes for SSH
...
def fetch_airflow_logs_size(ip: str, hostname: str, logs_path: str = AIRFLOW_LOGS_PATH) -> Dict:
    ...
    try:
        # Get directory size - properly parse du output
        ...
        if result.returncode == 0:
            ...
            # Get human readable size separately
            if is_local_check:
                human_cmd = f"du -sh {logs_path} | cut -f1"
            else:
                human_cmd = f"ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no rocky@{ip} \"du -sh {logs_path} | cut -f1\""
            
            human_result = subprocess.run(human_cmd, shell=True, capture_output=True, text=True, timeout=10)
            size_human = human_result.stdout.strip() if human_result.returncode == 0 else f"{size_mb}M"
...
@dag(
    dag_id='ha_service_health_monitor_v200',
    default_args={
        'owner': 'airflow',
    ...
    description='HA Infrastructure Health Monitor v200 with Resource Monitoring',
    schedule='*/2 * * * *',
    start_date=datetime(2025, 11, 1),
    catchup=False,
    tags=['monitoring', 'ha', 'health-check', 'v200'],
    max_active_runs=1,
)
def ha_service_health_monitor_v200():
    
    @task(task_id="detect_active_nfs")
    ...
    @task(task_id="system_resources")
    def check_system_resources():
        """Monitor system resources (CPU, RAM, Disk) and top processes"""
    ...
    @task(task_id="logs_size")
    def check_airflow_logs_size():
        """Monitor the size of /home/rocky/airflow/logs directory"""
    ...
    def create_service_check_task(server_name: str, service_name: str):
        """Factory function to create service check tasks with shortened names"""
        
        # Create short task ID
    ...
    def create_nfs_service_check_task(nfs_node_name: str, service_name: str, active_node_name: str):
        """Factory function to create NFS service check tasks with shortened names"""
        
        # Create short task ID
        service_short = SERVICE_ABBREV.get(service_name, service_name.replace('-', '_'))
        task_id = f"{nfs_node_name.replace('-', '')}_{service_short}"
            ...
            try:
                ...
                # Passive node: airflow-dag-processor should be inactive
                    ...
                    if service_name == 'airflow-dag-processor':
                        ...
                        # Other services can be active on passive
                        if service_is_active:
    ...
    def check_rabbitmq_cluster():
        ...
        for name, ip in nodes:
            try:
                # Use original credentials that worked
    ...
    @task(task_id="scheduler_heartbeat")
    ...
    def health_summary():
        ...
        if critical_failures:
            ...
            for task in critical_failures:
                ...
                print(f"     Check task logs for diagnostic information")
...
# Instantiate the DAG
dag_instance = ha_service_health_monitor_v200()
